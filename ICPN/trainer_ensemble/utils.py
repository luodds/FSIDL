...
def postprocess_args(args):            
    ...
    save_path1 = '-'.join([args.dataset, args.model_class, args.backbone_class, '{:02d}w{:02d}s{:02}q'.format(args.way, args.shot, args.query,"no val")])

    if args.distill:
        save_path2 = '_'.join(["Distill:" + args.model_name,
                               str(args.step_size),
                               'lr{:.2g}'.format(args.lr),
                               'T1{}T2{}'.format(args.temperature, args.temperature2),
                               '{}w_{}s'.format(args.way, args.shot)
                               ])
        # exchange unseen subsets
        args.subset_A = 'test'
        args.subset_B = 'val'

    else:
        save_path2 = '_'.join(["Ensemble:" + args.model_name,
                               str(args.lamb),
                               str('_'.join(args.step_size.split(','))),
                               str('wd:' + str(args.weight_decay)),
                               'lr{:.2g}'.format(args.lr),
                               'T1{}T2{}'.format(args.temperature, args.temperature2),
                               '{}w_{}s'.format(args.way, args.shot)
                               ])
    ...
    save_path1 += timestamp
    save_path2 += timestamp
...
def get_command_line_parser():
    ...
    parser.add_argument('--num_test_episodes', type=int, default=3000)
    parser.add_argument('--model_class', type=str, default='ProtoNet',
                        choices=['MatchNet', 'ProtoNet', 'BILSTM', 'FEAT']) # None for MatchNet or ProtoNet
    ...
    parser.add_argument('--fix_BN', action='store_true', default=True)     # means we do not update the running mean/var in BN, not to freeze BN
    parser.add_argument('--augment',  action='store_true', default=True)
    parser.add_argument('--multi_gpu', action='store_true', default=False)
    parser.add_argument('--gpu', default='2')
    parser.add_argument('--init_weights', type=str, default='/home/liuziyi/cub/saves/initialization/miniimagenet/Res12-pre.pth')
    ...
    parser.add_argument('--save_dir', type=str, default=os.getcwd() + '/checkpoints/subsets')
    parser.add_argument('--model_name', type=str, default='PN')

    # ensemble parameters (individuality / cooperation)
    parser.add_argument('--ensemble',  action='store_true', default=True)
    parser.add_argument('--trained_models', type=str, default=None)
    parser.add_argument('--lamb', type=float, default=0.5)
    parser.add_argument('--trade_off', action='store_true', default=True)
    parser.add_argument('--subset_A', type=str, default='train_b')
    parser.add_argument('--subset_B', type=str, default='train_a')
