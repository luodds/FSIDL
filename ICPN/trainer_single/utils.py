...
def postprocess_args(args):            
    ...
    save_path2 = '_'.join([args.model_name, str('_'.join(args.step_size.split(','))), str(args.gamma),
    ...
    if not os.path.exists(os.path.join(args.save_dir, save_path1)):
        os.mkdir(os.path.join((str(args.save_dir)), str(save_path1)))
...
def get_command_line_parser():
    ...
    parent_path = os.path.dirname(current_path)  # 获取父级路径
    path = os.path.dirname(parent_path)  # 获取父级路径
    parser.add_argument('--save_dir', type=str, default=path + '/cub/checkpoints/baseclass')
